...
/**
 * WebAssembly-JS implementation of DevIn Lexer
 *
...
actual class DevInLexerImpl : DevInLexer {
    ...
    actual override fun reset(input: String) {
        ...
        this.tokenIndex = 0
        this.currentState = DevInLexerStates.INITIAL
    }
            ...
            when {
                ...
                char == ':' -> {
                    if (input.startsWith("::", pos)) {
                        tokens.add(createToken(DevInTokenTypes.ACCESS, "::", startPos, startLine, startColumn))
                        currentColumn += 2
                        pos += 2
                    } else {
                        tokens.add(createToken(DevInTokenTypes.COLON, ":", startPos, startLine, startColumn))
                        currentColumn++
                        pos++
                    }
                }
                char == '(' -> {
                    tokens.add(createToken(DevInTokenTypes.LPAREN, "(", startPos, startLine, startColumn))
                    currentColumn++
                    pos++
                }
                char == ')' -> {
                    tokens.add(createToken(DevInTokenTypes.RPAREN, ")", startPos, startLine, startColumn))
                    currentColumn++
                    pos++
                }
                char == '[' -> {
                    tokens.add(createToken(DevInTokenTypes.LBRACKET, "[", startPos, startLine, startColumn))
                    currentColumn++
                    pos++
                }
                char == ']' -> {
                    tokens.add(createToken(DevInTokenTypes.RBRACKET, "]", startPos, startLine, startColumn))
                    currentColumn++
                    pos++
                }
                char == '{' -> {
                    tokens.add(createToken(DevInTokenTypes.OPEN_BRACE, "{", startPos, startLine, startColumn))
                    currentColumn++
                    pos++
                }
                char == '}' -> {
                    tokens.add(createToken(DevInTokenTypes.CLOSE_BRACE, "}", startPos, startLine, startColumn))
                    currentColumn++
                    pos++
                }
                char == ',' -> {
                    tokens.add(createToken(DevInTokenTypes.COMMA, ",", startPos, startLine, startColumn))
                    currentColumn++
                    pos++
                }
                char == '.' -> {
                    tokens.add(createToken(DevInTokenTypes.DOT, ".", startPos, startLine, startColumn))
                    currentColumn++
                    pos++
                }
                char == '|' -> {
                    if (input.startsWith("||", pos)) {
                        tokens.add(createToken(DevInTokenTypes.OROR, "||", startPos, startLine, startColumn))
                        currentColumn += 2
                        pos += 2
                    } else {
                        tokens.add(createToken(DevInTokenTypes.PIPE, "|", startPos, startLine, startColumn))
                        currentColumn++
                        pos++
                    }
                }
                char == '=' -> {
                    if (input.startsWith("==", pos)) {
                        tokens.add(createToken(DevInTokenTypes.EQEQ, "==", startPos, startLine, startColumn))
                        currentColumn += 2
                        pos += 2
                    } else if (input.startsWith("=>", pos)) {
                        tokens.add(createToken(DevInTokenTypes.ARROW, "=>", startPos, startLine, startColumn))
                        currentColumn += 2
                        pos += 2
                    } else {
                        tokens.add(createToken(DevInTokenTypes.TEXT_SEGMENT, "=", startPos, startLine, startColumn))
                        currentColumn++
                        pos++
                    }
                }
                char == '-' -> {
                    if (input.startsWith("->", pos)) {
                        tokens.add(createToken(DevInTokenTypes.PROCESS, "->", startPos, startLine, startColumn))
                        currentColumn += 2
                        pos += 2
                    } else {
                        tokens.add(createToken(DevInTokenTypes.TEXT_SEGMENT, "-", startPos, startLine, startColumn))
                ...
                char == '!' -> {
                    if (input.startsWith("!=", pos)) {
                        tokens.add(createToken(DevInTokenTypes.NEQ, "!=", startPos, startLine, startColumn))
                        currentColumn += 2
                        pos += 2
                    } else {
                        tokens.add(createToken(DevInTokenTypes.NOT, "!", startPos, startLine, startColumn))
                        currentColumn++
                        pos++
                    }
                }
                char == '<' -> {
                    if (input.startsWith("<=", pos)) {
                        tokens.add(createToken(DevInTokenTypes.LTE, "<=", startPos, startLine, startColumn))
                        currentColumn += 2
                        pos += 2
                    } else {
                        tokens.add(createToken(DevInTokenTypes.LT, "<", startPos, startLine, startColumn))
                        currentColumn++
                        pos++
                    }
                }
                char == '>' -> {
                    if (input.startsWith(">=", pos)) {
                        tokens.add(createToken(DevInTokenTypes.GTE, ">=", startPos, startLine, startColumn))
                        currentColumn += 2
                        pos += 2
                    } else {
                        tokens.add(createToken(DevInTokenTypes.GT, ">", startPos, startLine, startColumn))
                        currentColumn++
                        pos++
                    }
                }
                char == '&' -> {
                    if (input.startsWith("&&", pos)) {
                        tokens.add(createToken(DevInTokenTypes.ANDAND, "&&", startPos, startLine, startColumn))
                        currentColumn += 2
                        pos += 2
                    } else {
                        tokens.add(createToken(DevInTokenTypes.TEXT_SEGMENT, "&", startPos, startLine, startColumn))
                    ...
                    // Unknown character, treat as text segment
                    tokens.add(createToken(DevInTokenTypes.TEXT_SEGMENT, char.toString(), startPos, startLine, startColumn))
                    currentColumn++
                    pos++
                }
            }
